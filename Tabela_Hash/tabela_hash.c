...
hash * cria_hash(int TABLE_SIZE) {
	...
	if (ha != NULL) {
		...
		ha->listas = malloc(TABLE_SIZE * sizeof(Node*));
		if (ha->estudantes == NULL || ha->listas == NULL) {
			free(ha);
			return NULL;
		}
...
void libera_hash(hash * ha) {
	if (ha != NULL) {
		...
		//libera com endereçamento aberto
...
/**
 * Metodo de inserção, busca e romoção desse ponto para baixo.
 * Considerando o tratamento de colisões por encadeamento separado.
 */
...
char* busca_hash_enc_separado(hash *ha, int matricula) {
    ...
    Node *aux = ha->listas[pos];

    while (aux != NULL) {
        if (aux->chave->matricula == matricula)
            return aux->chave->nome;
        aux = aux->next;
    }
    return NULL;
}
